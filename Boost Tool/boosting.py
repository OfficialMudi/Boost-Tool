...
os.system(f"title Developed by Mudi")    
    ...
    try:
        for i in range(10):
            ...
            if response.status_code == 429:
                ...
                sprint(f"[{thread}] Captcha Beccato: {token}", False)
                r = response.json()
...
''')
print(f'''{Style.BRIGHT}{Fore.WHITE}                            Token: {len(open('input/3m_tokens.txt', 'r').read().splitlines())} Boost: {len(open('input/3m_tokens.txt', 'r').read().splitlines()) * 2}''' + Fore.RESET)	

def menu():
    invite = input(f"{Style.BRIGHT}{Fore.GREEN}[+]{Fore.WHITE}Invito: ")
    if ".gg/" in invite:
        invite = str(invite).split(".gg/")[1]
    elif "invite/" in invite:
        invite = str(invite).split("invite/")[1]
    if (
        '{"message": "invito sbagliato", "code": 10006}'
        in httpx.get(f"https://discord.com/api/v9/invites/{invite}").text):
        sprint("Invalid Invite Code", False)
        return

    try:
        months = int(input(f"{Style.BRIGHT}{Fore.GREEN}[+]{Fore.WHITE}Mesi: "))
    except:
        sprint("I mesi possono essere solo 1 o 3", False)
        return
    ...
    try:
        amount = (input(f"{Style.BRIGHT}{Fore.GREEN}[+]{Fore.WHITE}Quantita: "))
    except:
        sprint("Per poter boostare devi avere i token", False)
        return
    amount = int(amount)
    if amount % 2 != 0:
        sprint("Per poter boostare devi avere i token", False)
        return
    nick = input(f"{Style.BRIGHT}{Fore.GREEN}[+]{Fore.WHITE}nome: ")
    go = time.time()
    thread_boost(invite, amount, months, nick)
    end = time.time()
    time_went = round(end - go, 5)
    print()
    print(f"\n{Style.BRIGHT}{Fore.GREEN}Durata caricamento: {time_went} seconds\nSuccessful Boosts: {len(variables.success_tokens)*2}")
